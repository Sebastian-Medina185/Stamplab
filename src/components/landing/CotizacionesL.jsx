...
const CotizacionLanding = () => {
  ...
  const handleAgregarParte = () => {
    if (!disenoActual.TecnicaID || !disenoActual.ParteID) {
      Swal.fire("Atención", "⚠️ Selecciona técnica y parte", "warning");
      return;
    }
  ...
  const handleAbrirModalProductos = () => {
    if (traePrenda === "si") {
      if (!prendaDescripcion.trim()) {
        Swal.fire("Atención", "⚠️ Describe la prenda que traes", "warning");
        return;
      }
    } else {
      if (!formProducto.TipoTela || !formProducto.ColorID || !formProducto.TallaID) {
        Swal.fire("Atención", "⚠️ Completa todos los campos (Tela, Color, Talla)", "warning");
        return;
      }
    }
  ...
  const handleGenerarCotizacion = async (e) => {
    ...
    if (productosAgregados.length === 0) {
      Swal.fire("Atención", "⚠️ Agrega al menos un producto", "warning");
      return;
    }

    try {
      const detalles = productosAgregados.map(prod => ({
    ...
    <div className="container py-4">
      ...
      <div className="p-4 rounded shadow" style={{ backgroundColor: "#f5f5fa" }}>
        ...
        {traePrenda === "si" ? (
          <div className="mb-3">
            ...
            <textarea
              ...
              rows="3"
              placeholder="Describe la prenda que traes..."
        ...
        <div className="form-check mb-3">
          ...
          />
          <label className="form-check-label fw-bold fs-5" htmlFor="diseno">
            Aplicar diseño
          </label>
        </div>
        ...
        <div className="mb-3">
          <button
            ...
            onClick={handleAbrirModalProductos}
          >
            + Agregar Producto
          </button>
        </div>
